 ...
 * schema-traverse.ts
 *
 * Structured JSON Schema walker (self-contained, no external dependencies).
 ...
 *
 * Ported from executor/apps/web/src/lib/tool/schema-traverse.ts with the
 * json-schema-traverse dependency removed (we do our own recursive walk).
 */
...
export function resolveTypeLabel(schema: Record<string, unknown>): {
  type: SchemaFieldType;
  label: string;
  itemType?: string;
} {
...
export function traverseSchema(
  ...
  const walk = (
    ...
    // Handle top-level array schemas (or item-only schemas)
    if (Object.keys(properties).length === 0 && shape.items !== undefined) {
      const itemShape = asRecord(shape.items);

      if (Object.keys(itemShape).length > 0) {
        const itemPrefix = prefix ? `${prefix}[]` : "";
        walk(itemShape, itemPrefix, depth + 1, new Set<string>());
      }
    }
