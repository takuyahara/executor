...
const baseLoadedSourceCatalogTool = {
...
describe("source inspection", () => {
  it.effect("keeps the shared TypeScript contract first-class on tool detail", () =>
    Effect.gen(function* () {
      const detail = yield* inspectionToolDetailFromTool(
        structuredClone(baseLoadedSourceCatalogTool),
      );
  ...
  it.effect("truncates oversized executable sections to keep tool detail payload bounded", () =>
    Effect.gen(function* () {
      const tool = structuredClone(baseLoadedSourceCatalogTool);
      tool.executable.binding = {
        payload: "x".repeat(250_000),
      };

      const detail = yield* inspectionToolDetailFromTool(tool);
      const executableSection = detail.sections.find(
        (section) => section.kind === "code" && section.title === "Executable",
      );

      expect(executableSection).toBeDefined();
      if (!executableSection || executableSection.kind !== "code") {
        throw new Error("Expected Executable code section");
      }

      expect(executableSection.body).toContain(
        "Executable truncated for inspection UI",
      );
      expect(executableSection.body.length).toBeLessThanOrEqual(100_128);
      expect(executableSection.body).toContain("\"payload\":\"");
    }));
});
